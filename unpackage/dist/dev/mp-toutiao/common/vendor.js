...
function getLocaleLanguage$1() {
  let localeLanguage = "";
  {
    localeLanguage = normalizeLocale(tt.getSystemInfoSync().language) || LOCALE_EN;
  }
...
const offError = {
  args(fromArgs) {
    ...
    if (!app.$vm) {
      ...
      const index2 = tt.$onErrorHandlers.findIndex((fn) => fn === fromArgs);
      if (index2 !== -1) {
        tt.$onErrorHandlers.splice(index2, 1);
      }
...
function initRuntimeSocket(hosts, port, id2) {
  if (hosts == "" || port == "" || id2 == "")
  ...
  return hosts.split(",").reduce((promise, host) => {
    return promise.then((socket) => {
      ...
      return tryConnectSocket(host, port, id2);
    });
  }, Promise.resolve(null));
}
const SOCKET_TIMEOUT = 500;
function tryConnectSocket(host, port, id2) {
  return new Promise((resolve2, reject) => {
    const socket = index.connectSocket({
      url: `ws://${host}:${port}/${id2}`,
      multiple: true,
      // 支付宝小程序 是否开启多实例
      fail() {
        resolve2(null);
...
function initRuntimeSocketService() {
  const hosts = "127.0.0.1,192.168.1.12";
  const port = "8090";
  const id2 = "mp-toutiao_qIxfc5";
  ...
  return Promise.resolve().then(() => {
    ...
    return initRuntimeSocket(hosts, port, id2).then((socket) => {
      if (!socket) {
        restoreError();
        restoreConsole();
...
function initBaseInstance(instance, options) {
  ...
  ctx.$scope = options.mpInstance;
  {
    Object.defineProperties(ctx, {
      ...
      [VIRTUAL_HOST_ID]: {
        get() {
          const id2 = this.$scope.data[VIRTUAL_HOST_ID];
...
function initLifetimes$1({ mocks: mocks2, isPage: isPage2, initRelation: initRelation2, vueOptions }) {
  ...
  function attached() {
    ...
    this.$vm = $createComponent({
      ...
      mpType,
      mpInstance,
      slots: resolvePropValue(properties.uS) || {},
      // vueSlots
...
function initRelation(mpInstance, detail) {
  var _a, _b, _c;
  const nodeId = hasOwn(mpInstance, "__nodeId__") ? mpInstance.__nodeId__ : mpInstance.__nodeid__;
