...
import { getMyPapers } from "../../mock/home";
//查询参数
...
const searchData = async ()=>{
	try{
		...
		let res =  await getMyPapers(queryParams.value.page, queryParams.value.pageSize);
		classList.value  =  [...classList.value,...res] ;
		uni.setStorageSync("storgClassList",classList.value);	
		if(queryParams.value.pageSize > res.length) noData.value = true;
		if(res.length == 0 && classList.value.length==0) noSearch.value = true;
		console.log(res);
	}finally{
		uni.hideLoading()
	}
